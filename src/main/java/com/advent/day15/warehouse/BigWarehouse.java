    ...
    public BigWarehouse(Grid<Character> grid) {
        ...
        for (int x = 0; x < grid.maxX(); x++) {
            for (int y = 0; y < grid.maxY(); y++) {
                switch (grid.get(x, y)) {
                    case 'O' -> obstacles.add(Obstacle.bigBox(new Vector2((x * 2), y), new Vector2((x * 2) + 1, y)));
                    case '#' -> {
                        obstacles.add(Obstacle.wall(new Vector2((x * 2), y)));
                        obstacles.add(Obstacle.wall(new Vector2((x * 2) + 1, y)));
                    }
